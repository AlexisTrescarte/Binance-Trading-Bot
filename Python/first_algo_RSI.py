...
class RSI_Algo:
    ...
    def process_rsi(self):
        ...
        if (self.ema_50_list[-2]-self.ema_100_list[-2]<0) and (self.ema_50_list[-1]-self.ema_100_list[-1]>0) and (self.get_last_rsi() < 60) and not self.in_position[0]:
            self.want_buy = True
            self.buy()

        if self.get_last_rsi() < 30 and self.in_position[0]:
            self.want_sell = True
            self.sell()
